...
#define EXPECT_FALSE(x) std::cout<<x<<std::endl
#define EXPECT_TRUE(x) std::cout<<x<<std::endl
...
LegacyDelayBasedBweTest::LegacyDelayBasedBweTest()
    : clock_(100000000),
      ...
      stream_generator_(new StreamGenerator(1e6,  // Capacity.
...
void LegacyDelayBasedBweTest::AddDefaultStream() {
  stream_generator_->AddStream(new RtpStream(30, 3e5));
}
...
void LegacyDelayBasedBweTest::CapacityDropTestHelper(
  ...
  if (number_of_streams <= 1) {
    ...
    for (int i = 0; i < number_of_streams; i++) {
      ...
      stream_generator_->AddStream(new RtpStream(kFramerate, bitrate));
      bitrate_sum += bitrate;
    }
...
void LegacyDelayBasedBweTest::Test()
{
  ...
  std::cout<<bitrate_observer_.updated()<<std::endl;
}
}
int main()
{
	zsy::LegacyDelayBasedBweTest test;
	test.Test();
	return 0;
}
